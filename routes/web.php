...
Route::post('/updatedeliveryareatax', 'SimpleDeliveryController@updateDeliveryAreaTax')->name('updateDeliveryAreaTax')->middleware('isOwnerOnPro');

Route::match(['post', 'get'], '/email/verify/', 'RestorantController@verify_mail')->name('verification.notice');

Route::get('/price/{value}/{restorant}', 'FrontEndController@priceOnOff')->name('toggle-price');
Route::get('/', function () {
    return redirect()->to('login');
})->name('front');

Route::get('/pricing', 'PriceController@index')->name('pricing')->middleware(['universal', InitializeTenancyByDomain::class]);


Route::get('docs', 'QRController@documentation')->name('documentation');
...
Route::post('/get/hours', 'CartController@get_hours')->name('get_hours');

Route::group(['middleware' => ['auth', 'impersonate']], function () {

    Route::post('/awards/badges', 'RestorantController@awardsBadges')->name('award-badges');
    Route::get('/home/{lang?}', 'HomeController@index')->name('home')->middleware(['isOwnerOnPro', 'verifiedSetup']);
    Route::post('salesvalue', 'HomeController@salesData')->name('salesvalue')->middleware(['isOwnerOnPro', 'verifiedSetup']);
    ...
    Route::name('admin.')->group(function () {
        ...
        Route::name('restaurant.')->group(function () {
            ...
            // Areas
            Route::resource('businessareas', 'RestoareasController');
            Route::get('businessareas/del/{businessarea}', 'RestoareasController@destroy')->name('businessareas.delete');
            ...
            //Coupons
            Route::get('couponss', 'CouponsController@index')->name('coupons.index');
            ...
            //HappyHours
            //mass happy hours edit
            Route::get('happy_hours/edit', [\App\Http\Controllers\HappyHoursRestourantController::class, 'show'])->name('happyHours.show');

            Route::post('{restourant}/happy_hours/create', [\App\Http\Controllers\HappyHoursRestourantController::class, 'create'])->name('happyHours.create');
            Route::delete('{happyHours}/happy_hours/delete', [\App\Http\Controllers\HappyHoursRestourantController::class, 'delete'])->name('happyHours.delete');
        });
    });
    ...
    Route::post('/updateres/location/{restaurant}', 'RestorantController@updateLocation')->name('restaurant.updateLocation');
    ...
    Route::prefix('stripe')->name('stripe.')->group(function () {
        ...
        Route::get('/payment/oauth/{user}/onboard/', 'FinanceController@onboard')->name('onboarding');
    });
    ...
    Route::resource('clientss', 'ClientController');
    ...
    Route::resource('dynamic-price', 'DynamicPriceController');
    Route::post('dynamic-price/get', 'DynamicPriceController@getPrice')->name('dynamic-price.getPrice');
    Route::post('dynamic-price/get/form', 'DynamicPriceController@getForm')->name('dynamic-price.getForm');
    Route::post('dynamic-price/update/{id}', 'DynamicPriceController@update')->name('dynamic-price.update');
    ...
    Route::prefix('items')->name('items.')->group(function () {
        ...
        // HappyHours
        Route::post('HappyHours/{item}', 'HappyHoursController@store')->name('happyHours.store');
        Route::post('HappyHours/create/{item}', [\App\Http\Controllers\HappyHoursController::class, 'store'])->name('happyHours.create');
        Route::post('HappyHours/update/{item}', [\App\Http\Controllers\HappyHoursController::class, 'update'])->name('happyHours.update');
        Route::delete('HappyHours/del/{HappyHours}', 'HappyHoursController@destroy')->name('happyHours.destroy');
    });
    ...
    Route::get('qr/zip', 'QRController@zipCreate')->name('qr.zip');
    Route::post('qr/colour', 'QRController@qrColour')->name('qr.colour');
...
//Coupons for plans
Route::get('plancoupons/recalculateplan/{plan}', 'PlanCouponsController@recalculateplan')->name('plancoupons.recalculateplan');
Route::get('plancoupons', 'PlanCouponsController@index')->name('plancoupons.index');
Route::get('plancoupons/{coupon}/edit', 'PlanCouponsController@edit')->name('plancoupons.edit');
Route::get('plancoupons/create', 'PlanCouponsController@create')->name('plancoupons.create');
Route::post('plancoupons', 'PlanCouponsController@store')->name('plancoupons.store');
Route::put('plancoupons/{coupon}', 'PlanCouponsController@update')->name('plancoupons.update');
Route::get('plancoupons/del/{coupon}', 'PlanCouponsController@destroy')->name('plancoupons.delete');
Route::get('plancoupons/use/{coupon}', 'PlanCouponsController@use')->name('plancoupons.use');
...
Route::get('/new/' . config('settings.url_route') . '/register/{plan?}/', 'RestorantController@showRegisterRestaurant')->name('newrestaurant.register');
...
Route::get('/items/HappyHours/get/{item}', 'HappyHoursController@get')->name('HappyHours.get');
//Languages routes
...
Route::post('contactform/submit', 'RestorantController@submitContactForm')->name('submitcontactform')->middleware(['isOwnerOnPro', 'verifiedSetup']);

Route::get('contact/messages', 'RestorantController@contactMessages')->name('contactmessages')->middleware(['isOwnerOnPro', 'verifiedSetup']);

Route::post('compareplans', 'PriceController@compare_price')->name('compare_plans')->middleware(['isOwnerOnPro', 'verifiedSetup']);

Route::post('/{alias}/cart-add', function () {
    $i = 1;
})->where('alias', '.*');
