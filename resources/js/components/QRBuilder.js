...
export default class QRBuilder extends Component {
    ...
    render() {
        ...
        var QRStylesPart1=["QRNormal","QRRandRect","QR25D"];
        var QRStylesPart2=["QRBubble","QRLine"];
        return (
            <div className="row">
                            ...
                            <div className="row">
                                <div className="col-md-12" style={{ height: "160px", display: "flex", flexDirection:"row", justifyContent: "space-around"}}>
                                    {QRStylesPart1.map((value, index) => {
                                            return <div key={value}  onClick={()=>{this.setState({type:value})}} style={{...imageStyle, color:"black"}}><QR type={value} className="img-thumbnail" value={this.state.url} /></div>
                                    })}
