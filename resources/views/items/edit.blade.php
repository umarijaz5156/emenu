...
@section('content')
    @if ($errors->has('HappyHours'))
        <span class="invalid-feedback" role="alert">
            <strong>{{ $errors->first('HappyHours') }}</strong>
        </span>
    @endif
                ...
                <div class="modal-body p-0">
                    <div class="card bg-secondary shadow border-0">
                        <div class="card-body px-lg-5 py-lg-5">
                            <form role="form" method="post" action="{{ route('extras.store', $item) }}"
                                ...
                                @if ($item->has_variants)
                                    <div class="form-group">
                                        ...
                                        <label
                                            for="variantsSelector">{{ __('Select variant or leave empty for all') }}</label><br />
                                        <select name="variantsSelector[]" multiple="multiple"
                                            class="form-control noselecttwo" id="variantsSelector">
                                            @foreach ($item->uservariants as $variant)
                                                <option id="var{{ $variant->id }}" value="{{ $variant->id }}">
                                                    {{ '#' . $variant->id . ': ' . $variant->optionsList }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endif
                            ...
                            <div class="col-4 text-right">
                                @if (auth()->user()->hasRole('owner'))
                                    <a href="{{ route('items.index') }}"
                                        class="btn btn-sm btn-primary">{{ __('Back to items') }}</a>
                                @elseif(auth()->user()->hasRole('admin'))
                                    <a href="{{ route('items.admin', $restorant) }}"
                                        class="btn btn-sm btn-primary">{{ __('Back to items') }}</a>
                                @endif
                                @if (route::has('cloner.cloneitem'))
                                    <a href="{{ route('cloner.cloneitem', $item->id) }}"
                                        class="btn btn-sm btn-secondary">{{ __('Clone it') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group{{ $errors->has('item_name') ? ' has-danger' : '' }}">
                                            ...
                                            <input type="text" name="item_name" id="item_name"
                                                ...
                                                placeholder="{{ __('Name') }}"
                                                value="{{ old('item_name', $item->name) }}" required autofocus>
                                        ...
                                        <div class="form-group{{ $errors->has('item_description') ? ' has-danger' : '' }}">
                                            <label class="form-control-label"
                                                for="item_description">{{ __('Item Description') }}</label>
                                            <textarea id="item_description" name="item_description"
                                                class="form-control form-control-alternative{{ $errors->has('item_description') ? ' is-invalid' : '' }}"
                                                placeholder="{{ __('Item Description here ... ') }}" value="{{ old('item_description', $item->description) }}"
                                                required autofocus rows="3">{{ old('item_description', $item->description) }}</textarea>
                                        ...
                                        <div class="form-group{{ $errors->has('item_price') ? ' has-danger' : '' }}">
                                            <label class="form-control-label"
                                                for="item_price">{{ __('Item Price') }}</label>
                                            <input type="number" step="any" name="item_price" id="item_price"
                                                class="form-control form-control-alternative{{ $errors->has('item_price') ? ' is-invalid' : '' }}"
                                                placeholder="{{ __('Price') }}"
                                                value="{{ old('item_price', $item->price) }}" required autofocus>
                                        ...
                                        <script>
                                            document.addEventListener("DOMContentLoaded", () => {
                                                setInputFilter(document.querySelector("#discounted_price"), function(value) {
                                                    if (value < {{ $item->price }}) {
                                                        return true;
                                                    } else {
                                                        return false;
                                        ...
                                        @if ($restorant->getConfig('hide_tax_input', 'false') != 'false')
                                            ...
                                            @include('partials.input', [
                                                'id' => 'vat',
                                                'name' => __('VAT percentage( calculated into item price )'),
                                                'placeholder' => __('Item VAT percentage'),
                                                'value' =>
                                                    $item->vat == ''
                                                        ? $restorant->getConfig('default_tax_value', 0)
                                                        : $item->vat,
                                                'required' => false,
                                                'type' => 'hidden',
                                            ])
                                        ...
                                        @include('partials.input', [
                                            'id' => 'youtube',
                                            'name' => __('Youtube video/shorts URL'),
                                        ...
                                        @include('partials.toggle', [
                                            'id' => 'has_variants',
                                            'name' => __('Enable variants'),
                                            'checked' => $item->has_variants == 1,
                                        ])
                                        @if ($item->has_variants == 1)
                                            @include('partials.toggle', [
                                                'additionalInfo' => __(
                                                    'Missing variants will have the same price as the item'),
                                                'id' => 'enable_system_variants',
                                                'name' => __('Enable System Variants'),
                                                'checked' => $item->enable_system_variants == 1,
                                            ])
                                        @endif
                                        @if ($restorant->getConfig('stock_enabled', 'false') != 'false')
                                            @include('partials.toggle', [
                                                'id' => 'qty_management',
                                                'name' => 'Enable QTY Managment',
                                                'checked' => $item->qty_management == 1,
                                            ])
                                            ...
                                            @if ($item->qty_management == 1)
                                                @if ($item->has_variants == 1)
                                                    @include('partials.input', [
                                                        ...
                                                        'id' => 'qty',
                                                        'name' => __(
                                                            'Item QTY - determined as sum of the QTY of the variants'),
                            ...
                            <div class="text-center">
                                <form action="{{ route('items.destroy', $item) }}" method="post">
                                    ...
                                    <button type="button" class="btn btn-danger mt-4"
                                        onclick="confirm('{{ __('Are you sure you want to delete this item?') }}') ? this.parentElement.submit() : ''">{{ __('Delete') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-6 text-right">
                                    <a class="btn btn-secondary btn-sm"
                                        href="{{ route('items.options.index', $item->id) }}">{{ __('Edit Options') }}</a>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                    data-target="#modal-new-extras"
                                    onClick=(setRestaurantId({{ $restorant_id }}))>{{ __('Add') }}</button>
                            </div>
                        </div>
                    </div>
                    <div class="table-responsive">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                    ...
                                                    <button type="button" class="dropdown-item" data-toggle="modal"
                                                        data-target="#modal-new-extras"
                                                        onClick=(setExtrasId({{ $extra->id }}))>Edit</button>
                                                    <form action="{{ route('extras.destroy', [$item, $extra]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')

                                                        <button type="button" class="dropdown-item"
                                                            onclick="confirm('{{ __('Are you sure you want to delete this extras?') }}') ? this.parentElement.submit() : ''">
                                                            {{ __('Delete') }}
                                                        </button>
                                                    </form>
                                                </div>
