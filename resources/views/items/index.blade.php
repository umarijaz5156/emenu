                ...
                <div class="row align-items-center py-4">
                    ...
                    {{-- <div class="col-lg-12 col-12 text-right">
                        @if (config('settings.enable_miltilanguage_menus'))
                            <span
                                class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">{{ __('choose a menu language:') }}</span>
                                ...
                                <div class="row">
                                    <div class="col">
                                        ...
                                        <h3 class="mb-0">{{ __('Restaurant Menu Management') }} @if (config('settings.enable_miltilanguage_menus'))
                                            ...
                                            @endphp
                                            @if ($restorant->show_price == 1)
                                                <span class="mx-2 text-gray float-right">{{ __('Price is shown') }}</span>
                                                <a
                                                    href="{{ route('toggle-price', ['value' => 0, 'restorant' => $restorant_id]) }}">
                                                    <button style="float:right; margin-top: -10"
                                                        class="btn btn-primary">Hide Price</button>
                                                </a>
                                            @else
                                                <span class="mx-2 text-gray float-right">{{ __('Price is hidden') }}</span>
                                                <a
                                                    href="{{ route('toggle-price', ['value' => 1, 'restorant' => $restorant_id]) }}">
                                                    <button style="float:right;  margin-top: -10"
                                                        class="btn btn-danger">Show Price</button>
                                                </a>
                                            @endif
                                        </h3>
                                        <br />
                                        @if ($masseditpercent)
                                            <div class="alert alert-warning font-weight-bold" role="alert">
                                                {{ __('Warning:Mass Happy Hours implemented, discount') }} -
                                                {{ (int) $masseditpercent }}%
                                            </div>
                                        @endif
                                        @if (config('settings.enable_miltilanguage_menus'))
                                            <div class="alert alert-warning font-weight-bold" role="alert">
                                                {{ __('Now you can edit your menu in your language: ') }}
                                                ({{ $currentLanguage }})
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-auto">
                                            <div class="row">
                                                ...
                                                <div class="flex">
                                                    <form class="flex h-100"
                                                        action="{{ route('categories.destroy', $category) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')
                                                        <button class="btn btn-icon btn-1 btn-sm btn-danger"
                                                            type="button"
                                                    ...
                                                    <a href="{{ route('items.edit', $item) }}">
                                                        <div class="card">
                                                            ...
                                                            <div class="card-body">
                                                                ...
                                                                <span
                                                                    class="badge badge-primary badge-pill">@money($item->price, config('settings.cashier_currency'), config('settings.do_convertion'))</span>
                                                                @if ($item->happyHours->where('day', strtolower(date('l')))->count())
                                                                    <span class="badge badge-warning">Happy Hours
                                                                        Price</span>
                                                                ...
                                                                <p class="mt-3 mb-0 text-sm">
                                                                    @if ($item->available == 1)
                                                                        <span
                                                                            class="text-success mr-2">{{ __('AVAILABLE') }}</span>
                                                                    @else
                                                                        <span
                                                                            class="text-danger mr-2">{{ __('UNAVAILABLE') }}</span>
                                                                    @endif
                                                                </p>
                                                            </div>
                                                        </div>
