                        ...
                        <form id="{{ getFormId() }}" role="form" method="POST" action="{{ route('register') }}">
                            ...
                            <div class="form-group{{ $errors->has('phone') ? ' has-danger' : '' }}">
                                <div class="input-group input-group-alternative mb-3">
                                    ...
                                    <input class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}"
                                        placeholder="{{ __('Phone') }}" type="phone" name="phone"
                            ...
                            @if (config('settings.enable_birth_date_on_register'))
                                <div class="form-group">
                                    <div class="input-group input-group-alternative">
                                        ...
                                        <input class="form-control" placeholder="{{ __('Date of Birth') }}" id="birth_date"
                            ...
                            <div class="text-center">
                                @if (strlen(config('settings.recaptcha_site_key')) > 2)
                                    ...
                                    {!! htmlFormButton(__('Save'), [
                                        'id' => 'thesubmitbtn',
                                        'class' => 'btn btn-success mt-4',
                                        'disabled' => 'true',
                                    ]) !!}
                                @else
                                    <button disabled id="thesubmitbtn" type="submit"
                                        class="btn btn-primary mt-4">{{ __('Create account') }}</button>
                                @endif

                                <br /><br />
                                <div class="form-check"><input type="checkbox" name="termsCheckBox" id="termsCheckBox"
                                    ...
                                    <label for="terms" class="form-check-label text-gray-500">
                                        ...
                                        <a href="{{ config('settings.link_to_ts') }}" target="_blank"
                                            style="text-decoration: underline;">{{ __('terms_of_service') }}</a>
                                        {{ __('and') }}
                                        <a href="{{ config('settings.link_to_pr') }}" target="_blank"
                                            style="text-decoration: underline;">{{ __('privacy_policy') }}</a>.
                                    </label>
                                </div>
