                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('drivers.create') }}"
                                    class="btn btn-sm btn-primary">{{ __('Add driver') }}</a>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach ($drivers as $driver)
                                    <tr>
                                        ...
                                        <td>{{ $driver->created_at->format(config('settings.datetime_display_format')) }}
                                        </td>
                                        <td>
                                            @if ($driver->active == 1)
                                                    ...
                                                    <form action="{{ route('drivers.destroy', $driver) }}" method="post">
                                                        ...
                                                        @if ($driver->active == 0)
                                                            <a class="dropdown-item"
                                                                href="{{ route('driver.activate', $driver) }}">{{ __('Activate') }}</a>
                                                        @else
                                                            <button type="button" class="dropdown-item"
                                                                onclick="confirm('{{ __('Are you sure you want to deactivate this driver?') }}') ? this.parentElement.submit() : ''">
                                                                {{ __('Deactivate') }}
                                                            </button>
                                                        @endif
                                                    </form>
