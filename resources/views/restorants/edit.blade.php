        ...
        <div class="container-fluid">
            ...
            <div class="nav-wrapper">
                <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="res_menagment" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link mb-sm-3 mb-md-0 active " id="tabs-menagment-main" data-toggle="tab"
                            href="#menagment" role="tab" aria-controls="tabs-menagment" aria-selected="true"><i
                                class="ni ni-badge mr-2"></i>{{ __('Restaurant Management') }}</a>
                    </li>


                    @if (count($appFields) > 0)
                        <li class="nav-item">
                            <a class="nav-link mb-sm-3 mb-md-0 " id="tabs-menagment-main" data-toggle="tab" href="#apps"
                    ...
                    <li class="nav-item">
                        <a class="nav-link mb-sm-3 mb-md-0 " id="tabs-menagment-main" data-toggle="tab" href="#location"
                            role="tab" aria-controls="tabs-menagment" aria-selected="true"><i
                                class="ni ni-square-pin mr-2"></i>{{ __('Location') }}</a>
                    </li>
                    ...
                    @if (auth()->user()->hasRole('admin'))
                        <li class="nav-item">
                            <a class="nav-link mb-sm-3 mb-md-0" id="tabs-menagment-main" data-toggle="tab" href="#plan"
                                role="tab" aria-controls="tabs-menagment" aria-selected="true"><i
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="tab-content" id="tabs">
                    ...
                    <div class="tab-pane fade show active" id="menagment" role="tabpanel"
                        ...
                        <div class="card bg-secondary shadow">
                            <div class="card-header bg-white border-0">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-4 text-right">
                                        ...
                                        @endif
                                        @if ($hasCloner)
                                            <a href="{{ route('admin.restaurants.create') . '?cloneWith=' . $restorant->id }}"
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <form id="badge-upload-form" action="{{ route('award-badges') }}"
                                        enctype="multipart/form-data" class="col-12" method="post">
                                        @csrf
                                        <input type="hidden" name="restorant" value="{{ $restorant->id }}" />
                                        <div class="d-flex flex-wrap">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <div class="col-md-4 mb-4">
                                                    <label>Award Badge {{ $i }}</label>
                                                    <div class="badge-container">
                                                        <span class="small">{{ __('JPEG 100 x 100 recomended') }}</span>
                                                        <input type="file" name="badge_{{ $i }}"
                                                            class="badge-input form-control"
                                                            onchange="previewImage(this, 'badge_{{ $i }}_preview')">
                                                        @if ($restorant->{'badge_' . $i})
                                                            <img id="badge_{{ $i }}_preview"
                                                                src="{{ asset('storage/' . $restorant->{'badge_' . $i}) }}"
                                                                alt="Badge {{ $i }} Preview"
                                                                style="width: 100px; height: 100px;"
                                                                class="img-thumbnail badge-img">
                                                        @else
                                                            <img id="badge_{{ $i }}_preview" src="#"
                                                                alt="Badge {{ $i }} Preview"
                                        ...
                                        <div class="col-md-12">
                                            <div id="ajax_success_alert" class="alert alert-success"
                                                style="display: none;">

                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div id="ajax_error_alert" class="alert alert-danger" style="display: none;">
                                                Image size must be 500x500 px
                                            </div>
                                        </div>
                                        <div class="col-md-12 center-button">
                                            <button type="button" id="upload-button" class="btn btn-primary">Upload
                                                Badge Images <i id="indicatorStripe" style="display: none"
                                ...
                                <script>
                                    $(document).ready(function() {
                                        $('#upload-button').on('click', function() {
                                            ...
                                            var formData = new FormData($('#badge-upload-form')[0]);

                                            // Append the hidden field manually
                                            formData.append('restorant', $('#badge-upload-form input[name="restorant"]').val());

                                            var formAction = $('#badge-upload-form').attr('action'); // Get the form action URL
                                            $('#indicatorStripe').toggle();
                                            $.ajax({
                                                ...
                                                success: function(response) {
                                                    ...
                                                    // You might update the displayed images after successful upload
                                    ...
                                    function previewImage(input, previewId) {
                                        ...
                                        if (input.files && input.files[0]) {
                                            ...
                                            if (arrayExtensions.lastIndexOf(ext) === -1) {
                                                alert("Wrong extension type. we use only " + arrayExtensions.join(','));
                                                input.value = null;
                                                $(`#${previewId}`).attr('src', '#');
                                                $(`#${previewId}`).toggle();
                                                return;
                                            ...
                                            reader.onloadend = function(e) {
                                                ...
                                                preview.onload = function(e) {
                                                    if (preview.naturalWidth > 100 && preview.naturalHeight > 100) {
                                                        alert("Size must be not less than 100 x 100 px");
                                                        input.value = null;
                    ...
                    <!-- Tab Hours -->
                    <div class="tab-pane fade show" id="hours" role="tabpanel"
                        aria-labelledby="tabs-icons-text-1-tab">
...
@section('js')
    <!-- Google Map -->
    <script async defer src="https://maps.googleapis.com/maps/api/js?libraries=geometry,drawing&key=<?php echo config('settings.google_maps_api_key'); ?>">
    </script>
    ...
    <script type="text/javascript">
        ...
        $("input[type='checkbox'][name='days']").change(function() {
            var hourFrom = flatpickr($('#' + this.value + '_from' + "_shift" + $('#' + this.id).attr("valuetwo")),
                config);
            var hourTo = flatpickr($('#' + this.value + '_to' + "_shift" + $('#' + this.id).attr("valuetwo")),
                config);
