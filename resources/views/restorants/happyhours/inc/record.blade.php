...
<div class="row" >
    ...
    <div class="col-3">
        <div class="input-group">
            ...
            <input disabled="true" class = "text-center" style="border:1px solid gray" type="number" id="{{ 'itemprice_period_'.$perodDiscount->id }}" name="{{ 'period['.$key.']'.'[amount]' }}" value="{{ $perodDiscount->amount }}" class="flatpickr datetimepicker form-control" type="text">
            <input type="hidden" name="{{ 'period['.$key.']'.'[item_id]' }}" value="{{ $restorant->id }}"  type="text">
    ...
    <div class="col-1">
        <div class="input-group">
            @include('restorants.happyhours.inc.button_edit',compact(['restorant','perodDiscount']))
        </div>
    </div>
</div>
