    ...
    @if (count($order->items) > 0)
        ...
        <ul id="order-items">
            @foreach ($order->items as $item)
                ...
                @if ($item->pivot->qty > 0)
                    <li>
                        <h4>{{ $item->pivot->qty . ' X ' . $item->name }} - @money($theItemPrice, $currency, $convert) = ( @money($item->pivot->qty * $theItemPrice, $currency, true) )

                            @if ($item->pivot->vatvalue > 0)
                                <span class="small">-- {{ __('VAT') . ' ' . $item->pivot->vat . '%: ' }}
                                    (@money($item->pivot->vatvalue, $currency, true)) </span>
        ...
        @if (!empty($order->time_to_prepare))
            ...
            <br />
        @endif
        <h4>{{ __('Sub Total') }}: @money($order->order_price + $order->discount, $currency, $convert)</h4>
        {{-- <h5>{{ __('NET') }}: @money($order->order_price - ($item->vat / 100) * $item->pivot->variant_price, $currency, true)</h5> --}}
        @if ($order->discount > 0)
            <h4>{{ __('Discount') }}: -@money($order->discount, $currency, $convert)</h4>
            <h4>{{ __('Coupon code') }}: {{ $order->coupon }}</h4>
            <h4>{{ __('Sub Total After Discount') }}: @money($order->order_price, $currency, $convert)</h4>
        @endif



        <h5>{{ __('Tax') }} : @money($order->vatvalue, $currency, true)</h5>
        @if ($order->tip > 0)
        ...
        <h3>{{ __('Total') }} {{ $order->payment_method == 'cod' ? 'due' : 'paid' }} : @money($order->delivery_price + $order->order_price + $order->tip, $currency, true)</h3>
