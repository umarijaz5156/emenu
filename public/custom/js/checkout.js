...
window.onload = function () {
    ...
    initCOD();
    disableFunctions();
    crossCheckDeliveryPaymentMethod();
...
var disableFunctionsWP = function () {
    ...
    if (DISABLE_DELIVERY) {
        $("input:radio[name=deliveryType][value=delivery]").attr(
            "disabled",
            true
        );
    }
    if (DISABLE_PICKUP) {
        $("input:radio[name=deliveryType][value=pickup]").attr(
            "disabled",
            true
        );
    }
    if (DISABLE_DELIVERY || DISABLE_PICKUP) {
        $("input:radio[name=deliveryType]:not(:disabled):first").attr(
            "checked",
            true
        );
...
var disableFunctionsQR = function () {
    ...
    if (DISABLE_DELIVERY || DISABLE_PICKUP || DISABLE_DINEIN) {
        $("input:radio[name=dineType]:not(:disabled):first").attr(
            "checked",
            true
        );
...
var validateAddressInArea = function (positions, area) {
    ...
    if (area != null) {
        Object.keys(positions).map(function (key, index) {
            setTimeout(function () {
                var belongsToArea = google.maps.geometry.poly.containsLocation(
        ...
        getPlaceDetails(place_id, function (isFetched, data) {
            if (isFetched) {
                ...
                var mapAddress = new google.maps.Map(
                    document.getElementById("new_address_map"),
                    {
                        zoom: 17,
                ...
                var markerDataAddress = new google.maps.LatLng(
                    data.lat,
                    data.lng
                );
...
function crossCheckDeliveryPaymentMethod() {
    let deliveryTypeTakeAway = document.getElementById("deliveryTypeTakeAway");
    let deliveryTypeDinein = document.getElementById("deliveryTypeDinein");
    let deliveryTypeDerlivery = document.getElementById(
        "deliveryTypeDerlivery"
    );

    // Initialize the state based on the initial radio button selection
    updatePaymentMethod();

    // Add event listeners to the radio buttons
    deliveryTypeTakeAway.addEventListener("change", updatePaymentMethod);
    deliveryTypeDinein.addEventListener("change", updatePaymentMethod);
    deliveryTypeDerlivery.addEventListener("change", updatePaymentMethod);
}

function updatePaymentMethod() {
    let cashOnDelivery = document.getElementById("cashOnDelivery");

    if (deliveryTypeTakeAway.checked || deliveryTypeDerlivery.checked) {
        cashOnDelivery.disabled = true;
    }

    if (deliveryTypeDinein.checked) {
