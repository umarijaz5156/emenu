...
function updatePrices(net,delivery,enableDelivery){
  ...
  document.querySelector('#totalPriceVal').value = total.totalPrice;
  document.querySelector('#variantString').value = total.totalPrice;
}
...
window.onload = function () {
  ...
  footerPages = new Vue({
      el: '#footer-pages',
      data: {
        pages: [{link:"https://app.getterms.io/view/6LtCw/tos/en-au", title:"Terms and Conditions"},{link:"https://app.getterms.io/view/6LtCw/privacy/en-au", title:"Privacy Policy"}]
      },
  })
    ...
    methods: {
        addToCartAct() {
            axios.post('/cart-add', {
                ...
                variantID:variantID,
                price:  (typeof currentItemSelectedPrice !== 'undefined')?currentItemSelectedPrice:0,
                desc:   (typeof window.currentDesc !== 'undefined')?window.currentDesc:''
              })
              .then(function (response) {
                  if(response.data.status){
