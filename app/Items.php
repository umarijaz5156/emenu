...
class Items extends TranslateAwareModel
{
    ...
    public function getPriceAttribute($value){
        ...
        $value = $service->calculateRegularPrice($value,$this);
        return $value;
    }
    public function getDiscountedPriceAttribute($value){
        $service = new PriceCalculationService();
        $value = $service->calculateDiscountPrice($value,$this);
        return $value;
    }
    ...
    /** return all happyhours for this item
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function happyHours()
    {
        return $this->hasMany(HappyHours::class, 'item_id', 'id');
    }

    /** return all happyhours for this item that is availeble for this day and hour
    ...
    public function happyHoursNow()
    {
        ...
        $time = date('H:i');
        return $this->hasMany(HappyHours::class, 'item_id', 'id')->where('day',$day)->whereTime('start','<=',$time)->whereTime('end','>=',$time)->get();
    }
    ...
    {
        return $this->hasMany(DynamicPrice::class, 'item_id', 'id');
    }

    /** return all DynamicPrice for this item that is availeble for this day and hour
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function dynamicPriceNow()
    {
        $day = strtolower(date('l'));
        $time = date('H:i');
        return $this->hasMany(DynamicPrice::class, 'item_id', 'id')->where('day',$day)->whereTime('start','<=',$time)->whereTime('end','>=',$time)->get();
    }
}
