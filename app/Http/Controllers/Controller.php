...
class Controller extends BaseController
{
    ...
    public function getTimeSlotsForTheDay($vendor, $day){

        if ($day == date("Y-m-d",strtotime('now'))){
            $day = false;
        }
        ...
        if ($day){
            $now = new \DateTime($day, new \DateTimeZone($tz));
        }else{
            $now = new \DateTime('now', new \DateTimeZone($tz));
        }
        $arr = $businessHours->forDate($now);
        ...
        if ($day){
            ...
            $from = Carbon::now()->setTimezone($tz)->diffInMinutes(Carbon::now()->setTimezone($tz)->startOfDay());
        }

        foreach ($arr as $timeRange) {
            $start = $timeRange->start()->format('H')*60+$timeRange->start()->format('i');
            $end = $timeRange->end()->format('H')*60+$timeRange->end()->format('i');
        ...
        $intervalInMinutes = $vendor->getConfig('delivery_interval_in_minutes',config('settings.delivery_interval_in_minutes'));
        $intervals = [];
        foreach ($workingHoursArray as $time){
            for($i = $time['start']; $i<=$time['end']-$intervalInMinutes;$i+=$intervalInMinutes){
                $roundstart = round($i / $intervalInMinutes) * $intervalInMinutes;
                $roundend   = $roundstart+$intervalInMinutes;
                $stime = Carbon::today()->startOfDay()->addMinutes($roundstart)->format("H:i");
                $etime = Carbon::today()->startOfDay()->addMinutes($roundend)->format("H:i");
                $intervals[] = "{$stime} - {$etime}";
            }
        }
        return $intervals;
