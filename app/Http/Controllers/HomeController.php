...
class HomeController extends Controller
{
    ...
    public function index($lang = null)
    {
        ...
        if (config('settings.makePureSaaS', false) || (auth()->user()->hasRole('admin') && Module::has('pureadmindash'))) {
            //TODO:research on how this thing works
        ...
        if (auth()->user()->hasRole('admin')) {
            $last30daysOrders = Order::where('created_at', '>', $last30days)
                ->where('payment_status', 'paid')
                ->count();
        } else {
            $last30daysOrders = Order::where('restorant_id', auth()->user()->restorant->id)
                ->where('created_at', '>', $last30days)
                ->where('payment_status', 'paid')
                ->count();
        }
    ...
    public function salesData(Request $request)
    {
        ...
        // dd($request->all());
        $startDate = Carbon::parse($request->input('startDate'))->copy()->addDays(1);
        $endDate = Carbon::parse($request->input('endDate'));

        $dateRange = new DatePeriod($startDate, new DateInterval('P1D'), $endDate->modify('+1 day'));

        $data = new Collection([['Date', 'Order count', 'Sales Price ($)']]);

        // $admin_test = new Collection();

        foreach ($dateRange as $date) {
            $formated_date = $date->format('Y-m-d');

            if (auth()->user()->hasRole('admin')) {
                // $admin_test = 'yes admin';
                $orders = Order::select('id', 'delivery_price', 'static_fee', 'fee_value', 'created_at', 'order_price')
                    ->whereDate('created_at', $formated_date)
                    ->where('payment_status', 'paid')
                    ->get();
            } else {
                // $admin_test = 'no admin';
            ...
            $price = 0;
            // delivery_price + static_fee + fee_value
            foreach ($orders as $order) {
                // $admin_test->push($order->id);
