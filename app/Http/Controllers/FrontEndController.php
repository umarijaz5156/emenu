...
class FrontEndController extends Controller
{
    ...
    public function priceOnOff($value, $restorant)
    {

        $user = Auth::user()->id;
        $userTb = Restorant::findOrFail($restorant);
    ...
    public function index()
    {
        ...
        if ($customDomain != "") {
            return $this->restorant(request(), $customDomain);
        }
    ...
    public function restorant(Request $request, $alias)
    {
        //Do we have impressum app


        $qrParameter = $request->query('qr'); // This will return "1"
        ...
        $userplan = User::where('id', $restorant->user_id)->first();
        $plan = Plans::where('id', $userplan->plan_id)->first();



        if ($restorant && $qrParameter) {
            if ($plan->limit_qrcode == 0) {
                //   $restorant->qr_count += 1;
                // $restorant->update([
                //     'qr_count' => $restorant->qr_count,
                // ]);
            } else {
                if ($restorant->qr_count >= $plan->limit_qrcode) {
                    return abort(404, __('QrCode scan limit reached!'));
                } else {
                    $restorant->qr_count += 1;
                    $restorant->update([
                        'qr_count' => $restorant->qr_count,
        ...
        if ($restorant && $restorant->active == 1) {
            ...
            $viewData = [
                ...
                'showAllGTLanguages' => $restorant->getConfig('gt_all', false),
                'showGTLanguages' => $restorant->getConfig('gt_list', ""),
                'showLanguagesSelector' => config('settings.enable_miltilanguage_menus') && $restorant->localmenus()->count() > 1,
