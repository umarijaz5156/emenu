...
class OrderNotification extends Notification
{
    ...
    public function via($notifiable)
    {
        ...
        // dump('inside twilio notification from order');
        // $restorant = Restorant::findOrFail($this->order->restorant_id);
        // $restorant_configs = $restorant->getAllConfigs();
        // // dd('$restorant->all()', $restorant_configs['account_sid']);

        // if ($restorant_configs['account_sid'] && $restorant_configs['auth_token'] && $restorant_configs['from_number']) {
    ...
    public function toMail($notifiable)
    {
        ...
        if ($this->status . '' == '1') {
            ...
            $line = __('An exciting new order has been placed');
        } elseif ($this->status . '' == '3') {
            //Accepted
            $greeting = __('Your order has been accepted');
            $line = __('We are now working on it!');
        ...
        $message = (new MailMessage)
            ->greeting($greeting)
            ->subject($this->order->delivery_method . '' == '3' ? __('New Order Alert for Table') . ' ' . $this->order->table->name : __('New Order Alert'))
            ->line($line);
        ...
        $message->line(__('Ready to get moving?'));
        $message->action(__('View Order'), url(config('app.url') . '/orders/' . $this->order->id));
        $message->line(__('bon_appetit'));
