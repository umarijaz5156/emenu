...
class Restorant extends MyModel
{
    ...
    protected $fillable = ['name', 'badge_1', 'badge_2', 'badge_3', 'badge_4', 'badge_5', 'show_price', 'TripAdvisor', 'GoogleReview', 'subdomain', 'qr_count', 'user_id', 'lat', 'lng', 'address_1', 'address_2', 'city', 'zipcode', 'phone', 'logo', 'description', 'city_id', 'delivery_area_tax'];
    ...
    /**
        Gets current happyhours
     *
     */
    public function getCurrentHappyHoursPercent()
    {

        $current_time = strtotime(date("H:i:s"));
        $current_day_of_week = strtolower(date("l"));
        $all_hh = $this->happyHours();
        $ret = 0;
        $all_hh->each(function ($val, $key) use (&$ret, $current_day_of_week, $current_time) {
            $start = strtotime($val->start);
            $end = strtotime($val->end);
            if ($current_day_of_week == $val->day && $start < $current_time && $end > $current_time) {
                $ret += $val->amount;
            }
        });
        return $ret;
    ...
    public function rawplan()
    {
        $user = $this->user;
        return $user->belongsTo(Plans::class, 'plan_id');
    }

    /** return all happyhours for this restaurant
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function happyHours()
    {
        return $this->hasMany(HappyHoursForRestourant::class, 'restourant_id', 'id');
    }

    /** return all happyhours for this restaurant that is availeble for this day and hour
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function happyHoursNow()
    {
        $day = strtolower(date('l'));
        $time = date('H:i');
        return $this->hasMany(HappyHoursForRestourant::class, 'restourant_id', 'id')->where('day', $day)->whereTime('start', '<=', $time)->whereTime('end', '>=', $time)->get();
    }
}
