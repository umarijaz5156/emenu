...
class App
{
    ...
    private function printHeder()
    {
        ...
        $this->printer->feed();
        $this->printer->text($this->order->restorant->address);
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->text($this->order->restorant->phone);
        $this->printer->setEmphasis(false);
        $this->printer->feed();
    ...
    private function printItemsForKOT()
    {
        ...
        foreach ($this->order->items as $key => $item) {
            ...
            if (strlen($item->pivot->variant_name) > 3) {
                $this->printer->text(rtrim($this->columnify(__('Variant:'), str_replace("€", ' EUR', $item->pivot->variant_name), 30, 70, 32)) . "\n");
            }

            if (strlen($item->pivot->extras) > 3) {
                foreach (json_decode($item->pivot->extras) as $key => $extra) {
                    $this->printer->text(rtrim($this->columnify("", str_replace("€", ' EUR', $extra), 5, 95, 32)) . "\n");
                }
            }
        }
        $this->printer->feed();
    ...
    private function printItems()
    {
        ...
        $this->printer->selectPrintMode();
        $this->printer->setEmphasis(true);
        $this->printer->text(rtrim($this->columnify("Qty " . __('Item'), __('Price') . " (" . config('settings.cashier_currency') . ")", 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        foreach ($this->order->items as $key => $item) {
            ...
            if (strlen($item->pivot->variant_name) > 3) {
                $val = $this->convertUTF8(rtrim($this->columnify(__('Variant:'), str_replace("€", ' EUR', $item->pivot->variant_name), 30, 70, 32)));
    ...
    private function printTotals()
    {
        ...
        $this->printer->setEmphasis(true);
        $this->printer->text(rtrim($this->columnify(__('Sub Total'), config('settings.cashier_currency') . " " . ($this->order->order_price + $this->order->discount), 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        if ($this->order->coupon) {
            $this->printer->text(rtrim($this->columnify(__('Coupon'), config('settings.cashier_currency') . " " . $this->order->coupon, 60, 40, 32)));
            $this->printer->setEmphasis(false);
            $this->printer->feed();
            $this->printer->setEmphasis(true);
            $this->printer->feed();
        ...
        $this->printer->setEmphasis(true);
        $this->printer->text(rtrim($this->columnify(__('Tax'), config('settings.cashier_currency') . " " . $this->order->vatvalue, 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        // Tips
        if ($this->order->tip > 0) {
            $this->printer->text(rtrim($this->columnify(__('Tip'), config('settings.cashier_currency') . " " . $this->order->tip, 60, 40, 32)));
            $this->printer->setEmphasis(false);
            $this->printer->feed();
            $this->printer->setEmphasis(true);
            $this->printer->feed();
        ...
        if ($this->order->delivery_method == 1) {
            $this->printer->text(rtrim($this->columnify(__('Delivery'), config('settings.cashier') . " " . $this->order->delivery_price, 60, 40, 32)));
            $this->printer->setEmphasis(false);
            $this->printer->feed();
            $this->printer->setEmphasis(true);
            $this->printer->feed();
        ...
        if ($this->order->payment_status == 'paid') {
            $total_label = __('Total paid');
        } else {
            $total_label = __('Total due');
        }


        $this->printer->setEmphasis(true);
        $this->printer->text(rtrim($this->columnify($total_label, config('settings.cashier_currency') . " " . $this->order->order_price + $this->order->delivery_price + $this->order->tip, 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        // payment method
        $this->printer->text(rtrim($this->columnify(__('Payment method'), $this->order->payment_method, 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        // payment status
        $this->printer->text(rtrim($this->columnify(__('Payment status'), $this->order->payment_status, 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        // Dine method
        $this->printer->text(rtrim($this->columnify(__('Dine method'), $this->order->getExpeditionType(), 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
        ...
        // Time slot
        $this->printer->text(rtrim($this->columnify(__('Time slot'), $this->order->time_formated, 60, 40, 32)));
        $this->printer->setEmphasis(false);
        $this->printer->feed();
        $this->printer->setEmphasis(true);
        $this->printer->feed();
    ...
    private function convertUTF8(string $string)
    {
        return iconv(mb_detect_encoding($string, mb_detect_order(), true), "UTF-8//TRANSLIT//IGNORE", $string);
