    ...
    <div class="page-content container ">
        <div class="page-header text-blue-d2">
            ...
            <div class="action-buttons no-print">
                <a href="#" id="printBtn" class="btn bg-white btn-light mx-1px text-95" href="#"
                    data-title="Print">
                    <i class="mr-1 fa fa-print text-primary-m1 text-120 w-2"></i>
                    {{ __('Print invoice') }}
                </a>
                <a href="#" id="printReciptBtn" class="btn bg-white btn-light mx-1px text-95" href="#"
                    data-title="Print">
                    <i class="mr-1 fa fa-print text-primary-m1 text-120 w-2"></i>
                    {{ __('Print recipt') }}
                </a>
                <a href="{{ route('pdfinvoice.new', [$order]) }}" class="btn bg-white btn-light mx-1px text-95">
                    <i class="mr-1 fa fa-print text-primary-m1 text-120 w-2"></i>
                    {{ __('reprint receipt') }}
                </a>
                <a id="exportBtn" href="#" class="btn bg-white btn-light mx-1px text-95" href="#"
                    data-title="PDF">
                    <i class="mr-1 fa fa-file-pdf-o text-danger-m1 text-120 w-2"></i>
                    {{ __('Export') }}
                </a>
                <a style="display: none" id="sendBtn" href="#" class="btn bg-white btn-light mx-1px text-95"
                    href="#" data-title="PDF">
            ...
            <div class="col-12 col-lg-10 offset-lg-1">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <small
                            class="text-muted">{{ $order->created_at->format(config('settings.datetime_display_format')) }}</small><br />
                        <small class="text-muted">{{ __('Payment method') }}:
                            {{ __(strtoupper($order->payment_method)) }}</small>
                        <hr />
                    </div>
                </div>
                <div class="row">
                    ...
                    <div class="text-95 col-sm-6 align-self-start d-sm-flex justify-content-end">
                        ...
                        <div class="">
                            ...
                            @if ($order->restorant->getConfig('number_de_siren', false))
                                <h4>{{ __('Numéro de inscription') }}:
                                    {{ $order->restorant->getConfig('number_de_siren', '') }}</h4>
                                <h4>{{ __('Numéro Intracommunautaire') }}:
                                    {{ $order->restorant->getConfig('number_de_intercommunication', false) }}</h4>
                            @endif
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-12 col-sm-5 text-grey text-90 order-first order-sm-last">
                            ...
                            <div class="row my-2">
                                ...
                                <div class="col-5">
                                    <span class="text-110">@money($order->order_price + $order->discount, $currency, $convert)</span>
                                </div>
                            </div>
                            ...
                            @if ($order->discount > 0)
                                <div class="row my-2">
                                    ...
                                    <div class="col-5">
                                        <span class="text-110"> {{ $order->coupon }}</span>
                                        {{-- {{ print_r($order->coupon) }} --}}
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="row my-2">
                                <div class="col-7 text-right">
                                    <b>{{ __('Total') }} {{ $order->payment_method == 'cod' ? 'due' : 'paid' }} </b>
                                </div>
                                <div class="col-5">
                                    <b><span class="text-110 text-success-d3">@money($order->delivery_price + $order->order_price + $order->tip, $currency, true)</span></b>
                                </div>
                            </div>
